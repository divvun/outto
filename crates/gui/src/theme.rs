...
/// Windows 11-style focused button (blue border).
pub fn win11_button_focused(_theme: &Theme, status: button::Status) -> button::Style {
    let base = button::Style {
        background: Some(Color::from_rgb(0.898, 0.898, 0.898).into()),
        text_color: Color::from_rgb(0.122, 0.122, 0.122),
        border: Border {
            radius: 4.0.into(),
            width: 2.0,
            color: Color::from_rgb(0.0, 0.471, 0.831), // Windows blue
        },
        shadow: Default::default(),
        snap: false,
    };
...
/// Focus ring container style (thin blue border, no background).
...
/// Invisible focus ring (same size border, transparent — prevents layout shift).
