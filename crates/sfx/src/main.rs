// outto-sfx is the Windows PE self-extractor stub. On non-Windows hosts the
// crate builds to a placeholder binary so `cargo build --workspace` still
// succeeds — the macOS equivalent (a Mach-O extractor bundled inside an .app)
// will live in a separate crate when that pipeline is implemented.

#[cfg(not(windows))]
fn main() {
    eprintln!("outto-sfx: not implemented on this platform (Windows-only PE self-extractor)");
    std::process::exit(1);
}
...
mod imp {
    ...
    fn launch_and_wait(exe: &Path, args: &[String]) -> i32 {
        ...
        let mut pi = unsafe {
            std::mem::zeroed::<windows_sys::Win32::System::Threading::PROCESS_INFORMATION>()
        };
