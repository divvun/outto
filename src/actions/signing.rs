...
///
/// The command is executed via `cmd /C` on Windows, inheriting all environment
...
/// Sign all files that should be signed.
///
/// This includes any file with `codesign = true` in its entry,
/// plus any additional paths (setup, sfx, uninstaller) passed in.
pub fn sign_all(
    sign_command: &str,
    files: &[&Path],
    callbacks: &dyn InstallerCallbacks,
...
mod tests {
    ...
    fn test_sign_file_success() {
        ...
        let callbacks = crate::NoOpCallbacks;
    ...
    #[test]
    fn test_sign_file_failure_exhausts_retries() {
        let dir = std::env::temp_dir().join("outto_test_sign_fail");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        ...
        let callbacks = crate::NoOpCallbacks;
        // "exit 1" always fails
        let result = sign_file("exit /B 1", &file, &callbacks);
