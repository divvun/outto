...
fn main() {
    ...
    match build_installer(&config_path, &source_dir, &output) {
        Ok(()) => {}
        Err(e) => {
...
fn build_installer(
    ...
    let cli_dir = cli_exe.parent().ok_or("Cannot determine CLI exe directory")?;
    ...
    if let Some(ref p) = uninstall_exe {
        ...
        eprintln!("Warning: outto-uninstall binary not found. Installer will not include an uninstaller.");
    }
    ...
    pack_payload(config_path, source_dir, &temp_box_path, uninstall_exe.as_deref())?;

    let box_size = fs::metadata(&temp_box_path)?.len();
    eprintln!("Payload size: {} bytes ({:.1} MB)", box_size, box_size as f64 / 1_048_576.0);
...
fn pack_payload(
    ...
    let config_box_path = BoxPath::new("outto.toml")
